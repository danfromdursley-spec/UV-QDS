...
# QDS Avian Navigation Lab v3 PRO+ Builder & Launcher
# Hybrid Style • Species Mode • Noise Engine • Compass v3

set -e

ROOT=~/OMEGA_EMPIRE/UV_QDS/www
FILE=$ROOT/qds_avian_lab_v3_pro.html

echo "=== Building QDS Avian Magnetoreception Lab v3 PRO+ ==="
...
<h1>🦅 QDS Avian Magnetoreception Lab v3 PRO+</h1>
...
<div class="card">
  ...
  <div class="slider-label">τ<sub>c</sub> (ms) <span id="tcVal" class="value-pill"></span></div>
  <input id="tc" type="range" min="0.3" max="5" step="0.05" value="1.2">

  <div class="slider-label">λ<sub>c</sub> (nm) <span id="lcVal" class="value-pill"></span></div>
...
<div class="card">
  ...
  <div id="status" class="status warn">Initialising…</div>
</div>
...
<div class="card">
  ...
  <button class="btn" onclick="preset('forest')">🌲 Quiet Forest</button>
  <button class="btn" onclick="preset('urban')">🏙️ Urban Corridor</button>
  <button class="btn" onclick="preset('storm')">⛈️ Magnetic Storm</button>
  <button class="btn" onclick="preset('flare')">☀️ Solar Flare</button>
  <button class="btn" onclick="preset('lab')">🧪 Quantum Lab</button>
  <button class="btn-outline" onclick="toggle5G()">📡 Toggle 5G Tower</button>
</div>

<!-- EXPORT -->
<div class="card">
  <div class="section-title">Export Tools</div>
  <button class="btn-outline" onclick="snapshot()">📸 Export Snapshot</button>
</div>
...
function updateStatus(K) {
  if (K > 8) {
    statusBox.textContent="Stable Navigation — Ideal Conditions";
    statusBox.className="status ok";
  } else if (K > 4) {
    statusBox.textContent="Functional Navigation — Mild Noise";
    statusBox.className="status warn";
  } else if (K > 2) {
    statusBox.textContent="Compromised — Navigation Drift Likely";
    statusBox.className="status warn";
  } else {
    statusBox.textContent="Compass Collapse — Disorientation";
...
echo "[✔] v3 PRO+ HTML generated."

# launch server + open page
pkill -f "http.server" 2>/dev/null
python -m http.server 8011 --bind 127.0.0.1 &>/dev/null &
sleep 1
termux-open-url "http://127.0.0.1:8011/qds_avian_lab_v3_pro.html"

echo "[🚀] QDS Avian Lab v3 PRO+ launched."
